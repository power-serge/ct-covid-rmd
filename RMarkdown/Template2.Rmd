...
ddf <- data.frame(daily_cases,covid_county$dateupdated)
colnames(ddf)[2]<- "dateupdated"
...
d[is.na(d)] <- 0 
View()
sevenday_vector <- as.matrix(d[,2])
sevenday_vector[is.na(sevenday_vector)] <- 0
# View(d)
# write.table(sevenday_vector,row.names = FALSE,col.names = FALSE)
# View(sevenday_vector)
# nrow(sevenday_vector)
weekly_mean <- function(df){
  v1 <- c()
  for(i in 7:nrow(df)){
        v1[i-6] <- mean(df[(i-6):i])
  }
  return(v1) 
}

d$TotalCases
vec1 <- weekly_mean(sevenday_vector)
vec1
vec2 <- c()
vec2[1:7] <- NA
vec2[7:168] <- vec1
...
bar_cases <- ggplot(data = d, aes(x= dateupdated ,y = TotalCases, )) + 
  ...
  geom_line(aes(y = vec2), size = 1, color="red", group = 1) + theme_cowplot()
