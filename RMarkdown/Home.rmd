...
tcc <- ggplot(data = covid_mr, aes(x=county, y = CasesPer100K, fill = county)) +
  geom_bar(stat = "identity") + labs(x = "", y = "Total Cases (Per 100K)") + theme_classic() +
...
```{r, echo=FALSE, message=FALSE}
tdc <- ggplot(data = covid_mr, aes(x=county, y = DeathsPer100K, fill = county)) +
  geom_bar(stat = "identity") + labs(x = "", y = "Total Deaths (Per 100K)") + theme_classic() +
  theme(legend.position = "none") + theme(axis.text.x=element_text(angle = 45, hjust = 0.9, size = 11))
